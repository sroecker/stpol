 ...
 * of the same flavor. Applied to b- or c-quarks only. */
struct FlavourChain
{
    enum FlavourSource
    {
...
void FlavourAnalyzer::analyze(const edm::Event &event, const edm::EventSetup &eventSetup)
{
    ...
    // Find the chains (consecutive partons of the same flavor) in the particles collection
    ...
    for (list<GenParticle const *>::iterator partIt = particles.begin(); partIt != particles.end();)
    {
        ...
        //probably never have mothers of the same flavor, but it is not guaranteed.
        ...
        do
        {
            ...
            if (i == nMothers)  // i.e. the mother of the same flavor is not found
        ...
        // If the heavy flavor is in the initial section do not look for daughters
        ...
        while (currentParticle)
        {
            ...
            for (; i < nDaughters; ++i)
                ...
                // The first daughter of the same pdgId is saved only
                {
                    ...
                    //have more than one mother of the same flavor
            ...
            if (i == nDaughters)  // i.e. the daughter of the same flavor is not found
                currentParticle = NULL;
        }
      ...
      chainIt != flavourChains.end(); ++chainIt)
    {
        ...
        bool hasProtonAsParent = false;       // needed to determine the flavor source
        ...
        // Fill the lengths and parent. Also collect info for flavor source derivation
        ...
        // Make decision about the flavor source of the current chain
    ...
    }  // end of loop over the flavor chains
    
    
    // Now the flavor chains are almost filled. The only data member left to set is sisters
